...
def get_fc_output(model, x):
    """
    Get output from the final fully connected layer (before softmax)
    """
    with torch.no_grad():
        if x.dim() == 3:
            x = x.unsqueeze(0)
...
def process_and_attack_models(image_path, net, model_dict, transform, order_mapping, num_models):
    """
    Process image and run attacks on original and encrypted models
    """
    try:
        # Image processing
        ...
        # Get predictions from decrypted outputs
        with torch.no_grad():
...
if __name__ == "__main__":
    ...
    model_dict, order_mapping = ModifyModelVGGScale(
    ...
    # Setup statistics
    CSVfilenameTime = 'Model_Integrity_Results3.csv'
    ...
    with open(CSVfilenameTime, 'w', newline='') as fileobjT:
        ...
        Folder = r"/users/PMIU0184/guoj41/Classification/ILSVRC/Data/DET/train/folder3"
